...
import is.bobbys.gui.HudRenderer;
import is.bobbys.gui.MyGuiChat;
import is.bobbys.mod.*;
...
//        Bridge.tick();
        ...
        // 检查血量变化
        if(lastTarget != null) {
            float now = lastTarget.getHealth();
            if(now < targetHealth) {
                EntityPlayer victim = lastTarget;
                MyGuiChat.say(mc.thePlayer.getName()+" attack "+victim.getName()+" damage:"+(targetHealth - now));
                MyGuiChat.say("raw:"+DmgU.handDamage(mc.thePlayer));
    ...
    public void onBreakSpeed(PlayerEvent.BreakSpeed event){
        FireballTracer.breaking.add(event.pos);
    }

    @SubscribeEvent
    public void onExplosionDetonate(ExplosionEvent.Detonate event) {
        if (event.world != null && event.world.isRemote) {
            FireballTracer.onExplosion(event.getAffectedBlocks());
        }
    }
