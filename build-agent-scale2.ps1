...
$outputJar = Join-Path $outputDir 'fair-av-standalone-agent-1.8.9-fireball-gen-pro-esp-invis2.jar'
...
Get-ChildItem -Path (Join-Path $root 'src\agent\java') -Recurse -Filter '*.java' |
...
if (Test-Path -LiteralPath $emptyOrg) {
