...
public final class AutoTranslationPatcher {
    private static final String HOOK_CLASS = "com.example.autotranslation.agent.AutoTranslationHooks";
    ...
    public static byte[] patch(String className, byte[] bytes) {
        ...
        try {
            ...
            if (visitor.modifiedMethods() == 0) {
                AutoTranslationAgentLog.write("found no chat hook in " + className);
                return bytes;
            }
            AutoTranslationAgentLog.write("patched chat handler " + className);
            return writer.toByteArray();
        } catch (Throwable t) {
            AutoTranslationAgentLog.write("patch failed for " + className + ": " + t);
            return bytes;
        }
    }
